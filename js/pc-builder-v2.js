...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function openComponentSelector(component) {
        ...
        optionsGrid.innerHTML = config.options.map(option => `
            <div class="component-option-card ${option.recommended ? 'recommended' : ''}" data-component="${component}" data-name="${option.name}" data-price="${option.price}">
                ${option.recommended ? '<div class="recommended-badge"><i class="fas fa-star"></i> Recommended</div>' : ''}
                <div class="component-option-header">
                    <h4>${option.name}</h4>
                    <div class="component-option-price">${formatPrice(option.price)}</div>
                </div>
                <p class="component-option-specs">${option.specs}</p>
                <div class="component-option-footer">
                    <span class="component-brand">${option.brand}</span>
